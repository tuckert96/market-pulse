...
- current Alpha Engine evidence-gating checkpoint - recommendation confidence is capped/floored by explicit evidence quality, source freshness, thesis/history gaps, and risk urgency so thin-data opportunities cannot appear high confidence; use `git log` for the exact hash
