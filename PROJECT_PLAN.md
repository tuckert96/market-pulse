...
- current market-data fallback audit checkpoint - provider attempts now carry selected/fallback/sample provider roles, timestamps, cache/live/stale counts, quote counts, redacted safe error reasons, and Data Sources/ticker-page audit visibility; use `git log` for the exact hash
