...
- Optional fallback providers are controlled by `MARKET_DATA_FALLBACK_PROVIDERS`. Example: if Finnhub returns no usable quotes because of a rate limit/error and `financialModelingPrep` is configured with its own key, the backend can try FMP and reports the provider-attempt chain in Data Sources diagnostics and ticker-page data quality.
- Each market-data snapshot carries a safe `providerAttempts` audit trail. Attempts include provider id/label, role, status, timestamp, quote count, cache status, cache-hit/stale counts, and a redacted reason. API keys, raw tokenized URLs, cookies, and provider secrets must never appear in this trail.
