...
- current source history checkpoint - Imports and Data Sources now keep a safe reverse-chronological history of portfolio imports, provider syncs, market-data refreshes, backup restores, sample loads, and portfolio resets with redacted labels/counts only; use `git log` for the exact hash
