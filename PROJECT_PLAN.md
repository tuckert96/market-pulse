...
- current AI explanation review checkpoint - portfolio explanation responses now return side-by-side deterministic facts, optional generated wording, missing-context notes, source labels, and safety notes; Data Sources can request the review packet without exposing prompts, keys, or raw provider payloads; use `git log` for the exact hash
