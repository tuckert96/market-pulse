...
- current AI explanation source-contract checkpoint - OpenAI portfolio explanation prompts now use explicit allowlisted fields, stricter redaction, generated-output safety gating, provider-error URL scrubbing, and regression tests for no secrets, raw payloads, price targets, trade commands, return predictions, or unsupported causal claims; use `git log` for the exact hash
