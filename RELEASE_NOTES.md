...
- Seeking Alpha AI Research Coverage adds a dedicated queue for owned/watchlist/signal tickers, missing or stale imported context, parser warnings, change since prior import, source agreement/disagreement, and safe manual refresh prompts. It stays local/imported only and does not connect to Seeking Alpha, scrape pages, store credentials, or alter the core confluence score directly.
