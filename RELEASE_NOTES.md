...
- Market-data refreshes now expose a safe provider-attempt audit trail, including selected provider, configured fallback provider, sample fallback, quote counts, cache status, timestamps, stale-cache context, and redacted failure reasons in Data Sources and ticker-page data quality.
